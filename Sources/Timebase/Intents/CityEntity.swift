...
/// database so Siri/Shortcuts can pick one.
...
struct CityQuery: EntityStringQuery {
    func entities(for identifiers: [CityEntity.ID]) async throws -> [CityEntity] {
        ...
        return db.filter { identifiers.contains($0.id) }.map(CityEntity.init)
    }
    ...
    func suggestedEntities() async throws -> [CityEntity] {
        let db = City.loadBundled()
        return db.filter(\.popular).prefix(15).map(CityEntity.init)
    }
}
