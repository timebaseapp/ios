...
/// Verbatim port of the iOS OKLCH math — same matrices, same gamma curves —
/// so the Mac renders the exact same hues as iOS at every hour. This file is
/// the macOS target's own copy; tokens are duplicated per target by design.
struct OKLCH {
    var l: Double
    var c: Double
    var h: Double
...
/// OKLCH-based 24-hour time-of-day palette — the brand.
///
/// 14 light + 14 dark anchors, copied verbatim from `ios/.../Color/TimeColor.swift`.
/// The Mac uses light or dark depending on `\.colorScheme` (system theme on
/// macOS). Scrub-mute multiplies chroma when the wearer scrubs time, exactly
/// the iOS lever.
enum Palette {
    ...
    static let light: [Anchor] = [
        .init(hour: 0,    oklch: OKLCH(hex: "1E2538")),
        .init(hour: 3,    oklch: OKLCH(hex: "2A3548")),
        .init(hour: 5.5,  oklch: OKLCH(hex: "6A5F78")),
        .init(hour: 6.5,  oklch: OKLCH(hex: "F2DBA8")),
        .init(hour: 8,    oklch: OKLCH(hex: "F8E5BC")),
        .init(hour: 10,   oklch: OKLCH(hex: "F8D89E")),
        .init(hour: 12,   oklch: OKLCH(hex: "F8C788")),
        .init(hour: 14,   oklch: OKLCH(hex: "ECB070")),
        .init(hour: 16,   oklch: OKLCH(hex: "D89255")),
        .init(hour: 18,   oklch: OKLCH(hex: "C56E48")),
        .init(hour: 19.5, oklch: OKLCH(hex: "A55048")),
        .init(hour: 21,   oklch: OKLCH(hex: "5A4960")),
        .init(hour: 23,   oklch: OKLCH(hex: "2A3550")),
    ...
    static func background(forHour hour: Double, scheme: ColorScheme,
                           scrubMute: Double = 0) -> Color {
        Color(muted(base(forHour: hour, scheme: scheme), scrubMute: scrubMute).toRGBA(alpha: 1))
    }

    static func backgroundGradient(forHour hour: Double, scheme: ColorScheme,
                                   scrubMute: Double = 0) -> [Color] {
        let m = muted(base(forHour: hour, scheme: scheme), scrubMute: scrubMute)
    ...
    private static func muted(_ lch: OKLCH, scrubMute: Double) -> OKLCH {
        var out = lch
